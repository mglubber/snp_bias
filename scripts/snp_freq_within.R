...
positive_genomic_overlaps <- findOverlaps(snp_ranges,alu_positive,ignore.strand=TRUE)
negative_genomic_overlaps <- findOverlaps(snp_ranges,alu_negative,ignore.strand=TRUE)
positive_distance <- cbind.data.frame(alu_family=alu_positive$alufamily[subjectHits(positive_genomic_overlaps)], distance=abs(start(alu_positive)[subjectHits(positive_genomic_overlaps)] - start(snp_ranges)[queryHits(positive_genomic_overlaps)]))
negative_distance <- cbind.data.frame(alu_family=alu_negative$alufamily[subjectHits(negative_genomic_overlaps)], distance=abs(end(alu_negative)[subjectHits(negative_genomic_overlaps)] - end(snp_ranges)[queryHits(negative_genomic_overlaps)]))
