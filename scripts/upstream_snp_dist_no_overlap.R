...
flank_distance <- 200
positive_genomic_overlaps <- findOverlaps(snp_ranges,(flank(alu_positive,width=flank_distance,start=TRUE)),type="within",ignore.strand=TRUE)
negative_genomic_overlaps <- findOverlaps(snp_ranges,(flank(alu_negative,width=flank_distance,start=TRUE)),type="within",ignore.strand=TRUE)
